...
if (array_key_exists("taskid", $_GET)) {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'GET') {
        ...
        try {
            ...
            //Para crear el SQL  dinamiecamente del UPDATE
            $queryFields = "";
            ...
            if ($title_updated === true) {
                $task->setTitle($jsonData->title); //esto hace las valifaciones del title
                $up_title = $task->getTitle();
                $query->bindParam(':title', $up_title, PDO::PARAM_STR);
            }

            if ($description_updated === true) {
                $task->setDescription($jsonData->description); //esto hace las valifaciones del title
            ...
            //Para informar al usuario que se hizo el cambio
            $query = $writeDB->prepare("SELECT id, title, description, DATE_FORMAT(deadline, '%d/%m/%Y %H:%i') as deadline, completed FROM tbltasks WHERE id = :taskid");
            $query->bindParam(':taskid', $taskid, PDO::PARAM_INT);
            $query->execute();
            ...
            if ($rowCount === 0) {
                ...
                $response->setHttpStatusCode(404);
                $response->setSuccess(false);
                $response->addMessage("No Task found afeter update");
                $response->send();
                exit;
            }
            $taskArray = array();
            ...
            $response->setHttpStatusCode(500);
            $response->setSuccess(false);
            $response->addMessage("Failed to update - check your data for errors");
            $response->send();
            exit;
        }
    } else {
